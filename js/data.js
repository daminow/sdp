...
export function modal(type = 'text', val) {
    ...
    if (type === 'text') {
            ...
            for (const paymentValue of paymentType.values) {
                ...
                paymentValueItemInput.addEventListener('click', () => {
                    ...
                    if (paymentTypeInput.value === 'online' && paymentValueItemInput.checked && document.querySelector('.modal__form-payment-card-list') === null) {
                        ...
                        paymentCardDateInput.addEventListener('keydown', function (event) {
                            if (event.key != 'Backspace') {
                                paymentCardDateInput.value.replace(/\D/g, '');
                                if (event.key != 'Delete' && paymentCardDateInput.value.length === 2 && paymentCardDateInput.value.includes(' ') === false && paymentCardDateInput.value.includes('/') === false) {
                                ...
                                if (paymentCardDateInput.value.length === 7) {
                                    ...
                                    const cardYear = parseInt(paymentCardDateInput.value.slice(5));
                                    if (cardMonth < 1 || cardMonth > 12) {
                                        createAlertWindow('Месяц действия карты должен быть более 1 и менее 13', 5000)
                                        paymentCardDateInput.value = ''
                                        return
                                    }
                                    if (cardYear < 24 && cardYear > 50) {
                                        createAlertWindow('Год действия карты должен быть более 23 и менее 50', 5000)
                                        paymentCardDateInput.value = ''
                                        return
                                    } else {
        ...
        const completeBtn = createElement('button', 'Оформить заказ', ['btn-reset', 'modal__form-buy'], { type: 'submit' })
        completeBtn.addEventListener('click', (event) => {
            ...
            if (nameInput.value != '' && telInput.value.length === 16 && cityInput.value != '' &&
                ...
                setTimeout(() => {
                    ...
                    window.addEventListener('keydown', (e) => { if (e.key === "Escape") { closeModalEvent() } });
